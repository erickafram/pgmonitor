...
<head>
    ...
    <script>
        ...
        function gerarMaisSinais() {
            ...
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    var signals = JSON.parse(this.responseText);
                    if (signals.length > 0) {
...
<td><button class="btn btn-primary  btn-sm" onclick="pegarSinal(${signal.id})">Pegar Sinal</button></td>`;
                    ...
                    document.getElementById('gerarMaisSinaisMessage').innerText = signals.length > 0 ? 'Sinais indentificados com sucesso.' : 'Não há sinais com alta assertividade disponíveis. Por favor, tente novamente em alguns segundos...';
                    setTimeout(function() {
            ...
            xhttp.open("GET", "get_more_signals_3_sinais.php?t=" + Date.now(), true); // Adiciona um parâmetro de tempo exclusivo
            xhttp.send();
        }
