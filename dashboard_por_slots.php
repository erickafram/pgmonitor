...
<html>
<head>
    <title>Sistema de Sinais - Dashboard</title>
    ...
    <script>
        ...
        function gerarMaisSinais() {
            // Abrir o modal para escolher o nome do jogo
            $('#gameModal').modal('show');
        }

        function escanearJogo() {
            var gameName = document.getElementById('game_name_modal').value;
...
<td>
    ...
    <td><button class="btn btn-primary  btn-sm" onclick="pegarSinal(${signal.id})">Pegar Sinal</button></td>`;
                    ...
                    document.getElementById('gerarMaisSinaisMessage').innerText = signal !== null ? 'Sinal indentificado com sucesso.' : 'Não há sinais com alta assertividade disponíveis em plataformas pagantes. Por favor, tente novamente em alguns segundos...';
                    setTimeout(function() {
            ...
            xhttp.open("GET", "get_more_signals_slots.php?gameName=" + encodeURIComponent(gameName) + "&t=" + Date.now(), true); // Adiciona um parâmetro de tempo exclusivo
            xhttp.send();

            // Fechar o modal após escanear o jogo
...
<div class="container">
    ...
    <!-- Modal para escolher o nome do jogo -->
    <div class="modal fade" id="gameModal" tabindex="-1" aria-labelledby="gameModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="gameModalLabel">Scanner IA por Slot</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <label for="game_name_modal" class="form-label">Nome do Slot</label>
                    <select class="form-select" id="game_name_modal" name="game_name_modal" required>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" onclick="escanearJogo()">Escanear</button>
                </div>
            </div>
        </div>
    </div>
